...
namespace Final_Project
{
    public partial class history : Form
    {
        ...
        public history(main.logined_info cur_logined)
        {
        ...
        private void history_Load(object sender, EventArgs e)
        {
            ...
            oracleCommand1.CommandText = "SELECT info_books.i_name, r_date FROM rental_books, info_books where rental_books.i_id = info_books.i_id and m_id = '" + cur_logined.id + "' order by r_date";
            OracleDataReader rdr = oracleCommand1.ExecuteReader();
            int num = 0;
            while (rdr.Read())
            {
                num++;
                listBox_history.Items.Add(rdr["R_DATE"] + "\t" + rdr["I_NAME"]);
            }
            label_result.Text = "총 : " + num + "번";
        }
